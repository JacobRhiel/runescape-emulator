...
class UpdateBlockSet {
    ...
    val updateBlocks = EnumMap<UpdateBlockType, UpdateBlockStructure>(
        UpdateBlockType::class.java)
    ...
    fun load(properties: ServerProperties) {
        ...
        val excessMask = if (properties.has("excess-mask")) properties.get<Int>("excess-mask") else -1
        ...
        blocks.forEach { packet ->
            ...
            val bit = if (values.containsKey("bit")) values["bit"] else -1
            ...
            val structure = UpdateBlockStructure(bit = bit as Int, values = structureValues)
