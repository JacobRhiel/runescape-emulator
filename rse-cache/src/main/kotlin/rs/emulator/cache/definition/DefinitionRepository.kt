...
@Singleton class DefinitionRepository @Inject constructor()
{
    ...
    fun <T : Definition> find(clazz: Class<out T>, identifier: Int, child: Int): T?
    {
        ...
        var definitions = definitionCache.getIfPresent(clazz)
        ...
        if(definitions == null)
        {
            ...
            val reader = BufferedReader(fileStore.fetchArchiveFile(definition.indexConfig.identifier, if(child == -1) definition.archive else identifier, if(child == -1) identifier else child).contents)
        ...
        definitions = definitionCache.getIfPresent(clazz)!!
